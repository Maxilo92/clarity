(function() {
    ...
    function updateInsights() {
        ...
        if (subList) {
            if (subs.length === 0) {
                ...
                subList.innerHTML = subs.map(sub => `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem; background: rgba(255,255,255,0.5); border-radius: 8px;">
                        <div style="display: flex; flex-direction: column;">
                            <span style="font-size: 0.875rem; font-weight: 600; color: #1e293b;">${sub.name}</span>
                            <span style="font-size: 0.75rem; color: #64748b;">${sub.category} • Monthly</span>
                        </div>
                        <span style="font-size: 0.875rem; font-weight: 700; color: ${sub.amount < 0 ? '#e74c3c' : '#27ae60'};">
        ...
        // Trends
        const trends = window.ForecastEngine.calculateTrends(transactions, 'year');
        const trendsContent = document.getElementById('trends-content');
        if (trendsContent) {
            const expTrend = (trends.expenseTrend - 1) * 100;
            const revTrend = (trends.revenueTrend - 1) * 100;

            const getTrendColor = (val, isExpense) => {
                if (Math.abs(val) < 1) return '#64748b'; // Stable
                if (isExpense) return val > 0 ? '#e74c3c' : '#27ae60'; // Higher expenses is bad
                return val > 0 ? '#27ae60' : '#e74c3c'; // Higher revenue is good
            };
            ...
            trendsContent.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 12px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 0.875rem; color: #475569;">Monthly Expenses</span>
                        <span style="font-size: 0.875rem; font-weight: 700; color: ${getTrendColor(expTrend, true)};">
                            ${getTrendIcon(expTrend)} ${Math.abs(expTrend).toFixed(1)}% 
                            <small style="font-weight: 400; font-size: 0.75rem;">(vs avg)</small>
                        </span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 0.875rem; color: #475569;">Monthly Revenue</span>
                        <span style="font-size: 0.875rem; font-weight: 700; color: ${getTrendColor(revTrend, false)};">
                            ${getTrendIcon(revTrend)} ${Math.abs(revTrend).toFixed(1)}%
                            <small style="font-weight: 400; font-size: 0.75rem;">(vs avg)</small>
                        </span>
                    </div>
                    <div style="margin-top: 8px; padding-top: 12px; border-top: 1px dashed #e2e8f0;">
                        <p style="font-size: 0.8125rem; color: #64748b; line-height: 1.4;">
                            ${expTrend > 2 ? 'Your expenses are trending upwards. Consider reviewing your subscriptions.' : 
                              expTrend < -2 ? 'Great job! Your monthly spending is decreasing.' : 
                              'Your spending habits are currently stable.'}
                        </p>
                    </div>
                </div>
            `;
